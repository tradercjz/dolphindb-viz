...
import { TMovingStep } from '../hooks/useTMovingLogic';
import { COLORS } from '../constants';

interface TMovingStageProps {
  progress: number;
  steps: TMovingStep[];
  windowSize: number;
}

export const TMovingStage: React.FC<TMovingStageProps> = ({ progress, steps, windowSize }) => {
  ...
  return (
    <group position={[0, 0, 0]}>
        <Text position={[0, 4.5, 0]} fontSize={0.6} color="white" fontWeight="bold">
            tmoving(func, T, x, window={windowSize})
        </Text>
        ...
        <group position={[0, 3.5, 0]}>
             <Text fontSize={0.3} color={COLORS.secondary}>
                Window: ({windowStartVal}, {windowEndVal}]
             </Text>
        </group>
